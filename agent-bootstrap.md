...
1. Run `clawbooks doctor` first.
2. If no books exist, run `clawbooks init` and then inspect `.books/policy.md`.
3. Read `program.md` and `policy.md` before importing or reporting.
...
11. Use `policy.md` as the authority for basis, categorization, recognition, and review rules.
12. If policy is incomplete, say so explicitly and suggest the minimum policy updates needed.

Suggested first-run flow:

```text
Run `clawbooks doctor`.
Read `program.md`.
Read `clawbooks policy`.
